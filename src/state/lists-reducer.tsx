...
export type TodoListDomainType = TodoListType & { filter: FilterType, listStatus: ReqestStatusType }

type ListsStateType = TodoListDomainType[]

type ActionType = addListACType
    | removeListACType
...
export const listsReducer = (state: ListsStateType = initialState, action: ActionType): TodoListDomainType[] => {
    switch (action.type) {
        case "ADD LIST":
            const newList: TodoListDomainType = { ...action.payload.list, filter: 'all', listStatus: 'idle' };
            return [newList, ...state];
        case "REMOVE LIST":
            return state.filter(list => list.id !== action.payload.listID);
        case "CHANGE LIST FILTER":
            return state.map(list => list.id === action.payload.listID
                ? { ...list, filter: action.payload.newFilter } : list);
        case "CHANGE LIST TITLE":
            return state.map(list => list.id === action.payload.listID
                ? { ...list, title: action.payload.newTitle } : list);
        case "CHNAGE-LIST-STATUS":
            return state.map(list => list.id === action.listID ? { ...list, listStatus: action.newListStatus } : list);
        case "SET-LISTS":
            return action.payload.lists.map(list => ({ ...list, filter: 'all', listStatus: 'idle' }))
        case "CLEAR-LISTS":
            return state = [];
        default:
            return state;
...
export const changeListStatusAC = (listID: string, newListStatus: ReqestStatusType) => {
    return {
        type: 'CHNAGE-LIST-STATUS',
        listID, newListStatus
    } as const
}
        ...
        listsAPI.getLists()
            .then(response => {
                if (response.status === 200 && response.data.length) {
                    ...
                    dispatch(setErrorAC('Either no list has been found or an error occurred!'));
...
export const updateListTitleTC = (listID: string, newTitle: string) => {
    return (dispatch: Dispatch) => {
        dispatch(changeAppStatusAC('loading'));
        dispatch(changeListStatusAC(listID, 'loading'));
        listsAPI.editList(listID, newTitle)
            .then(response => {
                if (response.data.resultCode === 0) {
